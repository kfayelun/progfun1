...
class FunSetSuite extends FunSuite {
  ...
  test("Forall returns whether all bounded integers within `s` satisfy `p`.") {
    new TestSets {
      val s = forall((x: Int) => x > 3 && x < 8, (x: Int) => x > 0)
      assert(s, "forall 1")
      val t = forall((x: Int) => x > 3 && x < 8, (x: Int) => x < 0)
      assert(!t, "forall 2")
    }
  }
  ...
  test("Exists returns whether there exists a bounded integer within `s` that satisfies `p`.") {
    new TestSets {
      val s = exists((x: Int) => x > 3 && x < 8, (x: Int) => x > 0)
      assert(s, "exists 1")
      val t = exists((x: Int) => x > 3 && x < 8, (x: Int) => x < 0)
      assert(!t, "exists 2")
    }
  }
  ...
  test("Map returns a set transformed by applying `f` to each element of `s`.") {
    new TestSets {
      val s = map(s1, (x: Int) => 2*x)
      assert(!contains(s, 1), "map 1")
      assert(contains(s, 2), "map 1b")
      val t = map(s2, (x: Int) => x*x)
